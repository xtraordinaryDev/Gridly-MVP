...
import { CreateBuyerAccountSchema, type CreateBuyerAccountValues } from "./schema"
...
export function CreateAccountForm({
  ...
  const form = useForm<CreateBuyerAccountValues>({
    resolver: zodResolver(CreateBuyerAccountSchema),
    defaultValues: { password: "", confirmPassword: "" },
  })

  function onSubmit(values: CreateBuyerAccountValues) {
    startTransition(async () => {
      const res = await createBuyerAccount(token, values)
      if (res.ok) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        <div>
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">
            This is the email from your approved request.
          </p>
        </div>
